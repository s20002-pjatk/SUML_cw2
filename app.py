...
def main():
	...
	with left:
		...
		ilness_radio = st.radio( "Czy chory?", list(illness_d.keys()), format_func=lambda x: illness_d[x])
		#embarked_radio = st.radio( "Port zaokrętowania", list(embarked_d.keys()), index=2, format_func= lambda x: embarked_d[x] )

	with right:
		age_slider = st.slider("Wiek", value=50, min_value=1, max_value=100)
		height_slider = st.slider( "Wzrost", min_value=40, max_value=220)
	...
	data = [[ objawy_radio,age_slider,ilness_radio, height_slider ]]
